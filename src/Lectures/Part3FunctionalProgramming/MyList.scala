...
abstract class MyList[+T] {
  ...
  def map[S >: T, P](trans: S => P): MyList[P]
  def filter[S >: T](pred: S => Boolean): MyList[S]
  def flatMap[S>:T, P](trans: S => MyList[P]): MyList[P]
...
case object Empty extends MyList[Nothing] {
  ...
  override def map[S >: Nothing, P](trans: S => P): MyList[P] = Empty
  override def filter[S >: Nothing](pred: S => Boolean): MyList[S] = Empty
  override def flatMap[S >: Nothing, P](trans: S => MyList[P]): MyList[P] = Empty
...
case class Cons[+T](h: T, t: MyList[T]) extends MyList[T] {
  ...
  override def map[S >: T, P](trans: S => P): MyList[P] = new Cons(trans(h), t.map(trans))
  override def filter[S >: T](pred: S => Boolean ): MyList[S] =
    if (pred(h)) new Cons(h, t.filter(pred))
    else t.filter(pred)
  override def flatMap[S >: T, P](trans: S => MyList[P]): MyList[P] = trans(h) ++ t.flatMap(trans)
...
object ListTest extends App {
  ...
  println(listofIntegers.flatMap((x: Int) => new Cons(x, new Cons(x, Empty))).toString)

  println(listofIntegers.flatMap((v1: Int) => new Cons(v1, new Cons(v1, Empty))).filter((v1: Int) => ((v1 % 2) == 1)).toString)
